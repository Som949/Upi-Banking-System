...
export default function DepositMoney() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (res.success) {
				setSuccess(`✅ Rs. ${amount} deposit successfully! New Balance: Rs. ${res.data.new_balance}`)
				setAmount('')
			} else {
				setError(res.message || 'Money is not Deposited')
			}
		} catch {
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div>
							...
							<input
								...
								className="w-full rounded-lg border border-slate-600/50 bg-slate-800/30 px-4 py-3 text-slate-200 placeholder-slate-600 outline-none focus:border-emerald-500/50"
							/>
						</div>
						...
						{/* Daily Limit Info */}
						<div className="rounded-lg border border-slate-600/30 bg-slate-800/20 p-4">
							<p className="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-2">DAILY LIMIT</p>
							<p className="text-sm text-slate-200">Max Rs. 1,00,000 per day</p>
						</div>
						...
						<button
							...
							className="w-full rounded-full bg-linear-to-r from-emerald-500 to-emerald-600 py-3 text-lg font-semibold text-white transition hover:from-emerald-600 hover:to-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed"
						>
