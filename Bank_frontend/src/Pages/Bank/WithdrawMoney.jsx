...
export default function WithdrawMoney() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (res.success) {
				setSuccess(`✅ Rs. ${amount} successfully withdraw! New Balance: Rs. ${res.data.new_balance}`)
				setAmount('')
			} else {
				setError(res.message || 'No Withdraw')
			}
		} catch {
			...
			<div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
				...
				<div className="rounded-2xl border border-slate-700/50 bg-linear-to-br from-slate-900/70 to-slate-950/70 p-8 backdrop-blur-sm">
					<div className="mb-8 flex items-center gap-4">
						...
						<div>
							<h2 className="text-2xl font-bold text-slate-100">Withdraw Money</h2>
							<p className="text-sm text-slate-400">Customer ke account se cash nikalo</p>
						</div>
					</div>
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						{/* Daily Limit Info */}
						<div className="grid grid-cols-2 gap-4 rounded-lg border border-slate-600/30 bg-slate-800/20 p-4">
							<div>
								<p className="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-1">DAILY LIMIT</p>
								<p className="text-sm text-slate-200">Rs. 1,00,000</p>
							</div>
							<div>
								<p className="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-1">MAX PER TXN</p>
								<p className="text-sm text-slate-200">Rs. 1,00,000</p>
							</div>
						</div>
						...
						{/* Success */}
						{success && (
							<div className="rounded-lg border border-emerald-400/30 bg-emerald-400/15 px-4 py-3 text-sm text-emerald-200">
								{success}
							</div>
						)}
						...
						<button
							...
							className="w-full rounded-full bg-linear-to-r from-blue-500 to-blue-600 py-3 text-lg font-semibold text-white transition hover:from-blue-600 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
						>
