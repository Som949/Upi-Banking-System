...
// 			if (res.success) {
// 				// ✅ ADDED: token aur default password status localStorage mein save karo
// 				localStorage.setItem('bank_token', res.token)
// 				localStorage.setItem('is_default_password', res.is_default_password)
...
// 				}
// 			} else {
// 				setError(res.message || 'Galat password')
// 			}
// 		} catch {
...
const AdminLogin = () => {
	// ✅ ADDED: navigate, state hooks — function ke ANDAR move kiye (pehle bahar the jo error tha)
	const navigate = useNavigate()
	const [password, setPassword] = useState('')
	const [error, setError] = useState('')
	const [loading, setLoading] = useState(false)
			...
			<section className="relative mx-auto w-full max-w-6xl">
				<div className="overflow-hidden rounded-[28px] border border-[#1F2C45] bg-[#030E21]/80 shadow-[0_30px_100px_rgba(0,0,0,0.55)] backdrop-blur-xl lg:grid lg:grid-cols-[1.1fr_1fr]">
					<div className="relative flex min-h-140 flex-col justify-between border-b border-[#1E2A43] bg-linear-to-br from-[#081833] via-[#061628] to-[#04101F] p-7 sm:p-10 lg:border-b-0 lg:border-r">
						...
						<div className="relative z-10 mt-10 flex items-center gap-4 rounded-xl border border-[#2A3D65] bg-[#071A34]/70 px-4 py-3 text-sm text-[#C9D9FF] backdrop-blur">
							<div className="flex -space-x-2">
								<div className="grid h-7 w-7 place-items-center rounded-full border border-[#1A2C4D] bg-[#12335E] text-[10px] font-semibold text-[#D9E7FF]">AK</div>
								<div className="grid h-7 w-7 place-items-center rounded-full border border-[#1A2C4D] bg-[#234E82] text-[10px] font-semibold text-[#D9E7FF]">DS</div>
							</div>
							<p>
								<span className="font-semibold text-[#79F7CF]">128 Secure Sessions</span>{' '}
								active globally
					...
					<div className="flex min-h-140 flex-col justify-between bg-[#050E1D]/90 p-7 sm:p-10">
						<div>
							...
							{/* ✅ UPDATED: onSubmit handler add kiya */}
							<form className="mt-10 space-y-5" onSubmit={handleLogin}>
								<div>
									<label htmlFor="adminPassword" className="mb-2 block text-xs font-semibold uppercase tracking-[0.28em] text-[#7E93BA]">
										Secure Password
									</label>
									<div className="group flex items-center gap-3 rounded-xl border border-[#2A3A59] bg-[#15233B] px-4 py-3 transition focus-within:border-[#5D83FF] focus-within:ring-2 focus-within:ring-[#5D83FF]/35">
										<span className="text-[#8AA3CC]"><LockIcon /></span>
										{/* ✅ UPDATED: value, onChange, type toggle add kiya */}
										<input
											id="adminPassword"
											...
											className="w-full bg-transparent text-sm tracking-[0.2em] text-white placeholder:text-[#6C81A8] outline-none"
										/>
										{/* ✅ UPDATED: show/hide toggle kaam karta hai ab */}
										<button
											type="button"
								...
								<div className="flex items-start gap-3 rounded-xl border border-[#2A3A59] bg-[#111F35]/80 px-4 py-3 text-xs leading-5 text-[#9FB0CF]">
									<span className="mt-0.5 text-[#72F2BE]"><ShieldIcon /></span>
									<p>Two-factor authentication will be required on the next step for verified IP addresses.</p>
								</div>

								{/* ✅ ADDED: error message */}
								{error && (
									<p className="rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-2 text-center text-sm text-red-400">
										{error}
									</p>
								)}

								<div className="space-y-3 pt-3">
									{/* ✅ UPDATED: disabled aur loading state add kiya */}
									<button
										type="submit"
										disabled={loading || !password}
										className="w-full rounded-full bg-linear-to-r from-[#587FEF] to-[#6E95FF] px-5 py-3 text-sm font-semibold text-[#0A1730] shadow-[0_10px_28px_rgba(94,139,255,0.45)] transition hover:from-[#6E95FF] hover:to-[#7BA4FF] disabled:opacity-60 disabled:cursor-not-allowed"
									>
