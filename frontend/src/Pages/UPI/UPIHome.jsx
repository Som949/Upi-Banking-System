  ...
  return (
    <>
      <style>{`
        ...
        .vault-login-active:active { transform: scale(0.98); }
        .login-btn-inner:hover { transform: scale(1.05); }
      `}</style>

      <div className="vault-mesh" style={{ position: "fixed", inset: 0, pointerEvents: "none" }} />
      <div className="vault-gradient" style={{ position: "fixed", inset: 0, pointerEvents: "none" }} />

      <main style={{
        position: "relative", zIndex: 10,
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
        minHeight: "100vh", padding: "0 1.5rem", textAlign: "center",
        backgroundColor: "var(--color-surface-dim)", fontFamily: "Inter, sans-serif",
        color: "var(--color-on-surface)", overflow: "hidden",
      }}>
        {/* Branding */}
        <div style={{ marginBottom: "4rem" }}>
          <div style={{ position: "relative", display: "inline-block", marginBottom: "1.5rem" }}>
            <div style={{ position: "absolute", inset: 0, background: "rgba(108,136,255,0.2)", filter: "blur(48px)", borderRadius: "9999px" }} />
            <div style={{ position: "relative", background: "var(--color-surface-container-highest)", padding: "1.25rem", borderRadius: "1.5rem", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.5)" }}>
              <span className="material-symbols-outlined" style={{ color: "var(--color-primary-fixed-dim)", fontSize: "3rem", fontVariationSettings: "'FILL' 1" }}>lock</span>
            </div>
          </div>
          <div>
            <h1 style={{ fontFamily: "Manrope, sans-serif", fontWeight: 800, fontSize: "3rem", letterSpacing: "-0.025em", color: "white", margin: 0 }}>VAULT</h1>
            <p style={{ fontFamily: "Inter, sans-serif", color: "var(--color-on-surface-variant)", fontSize: "1.125rem", fontWeight: 500, letterSpacing: "0.05em", marginTop: "0.5rem" }}>
        ...
        <div style={{ width: "100%", maxWidth: "24rem" }}>
          ...
          <button
            ...
            onClick={() => navigate("/upi/login")}
            style={{
              position: "relative", width: "100%", padding: "1.25rem 2rem",
              borderRadius: "0.75rem", fontFamily: "Manrope, sans-serif", fontWeight: 700,
              fontSize: "1.125rem", overflow: "hidden", transition: "all 0.3s",
              border: "none", cursor: "pointer", marginBottom: "1rem", display: "block",
            }}
          >
            <div className="login-btn-inner" style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, var(--color-primary-container), var(--color-primary))", transition: "transform 0.3s" }} />
            <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.75rem", color: "var(--color-on-primary-fixed)" }}>
              <span>Login</span>
              <span className="material-symbols-outlined" style={{ fontSize: "1.25rem" }}>arrow_forward</span>
            </div>
          </button>

          {/* ✅ Create Account → /upi/register */}
          <button
            className="vault-login-active"
            onClick={() => navigate("/upi/register")}
            style={{
              width: "100%", padding: "1.25rem 2rem", borderRadius: "0.75rem",
              fontFamily: "Manrope, sans-serif", fontWeight: 700, fontSize: "1.125rem",
              transition: "all 0.3s", background: "rgba(27, 31, 43, 0.4)",
              border: "1px solid rgba(68, 70, 85, 0.3)", color: "var(--color-primary)", cursor: "pointer",
            }}
        ...
        <div style={{
          ...
          color: "rgba(196,197,215,0.6)", fontSize: "0.75rem",
      ...
      <div style={{ position: "fixed", top: "-10%", left: "-10%", width: "40%", height: "40%", background: "rgba(184,196,255,0.05)", filter: "blur(120px)", borderRadius: "9999px", pointerEvents: "none" }} />
      <div style={{ position: "fixed", bottom: "-10%", right: "-10%", width: "40%", height: "40%", background: "rgba(226,113,26,0.05)", filter: "blur(120px)", borderRadius: "9999px", pointerEvents: "none" }} />
    </>
  );
}